...
function updateInfoPanel(contact) {
  ...
  if (grid && sec) {
    if (contact.animals && contact.animals.length) {
      grid.innerHTML = contact.animals.map(a => '<div class="info-animal-card"><span class="animal-emoji">' + a.emoji + '</span><div class="animal-name">' + a.name + '</div><div class="animal-breed">' + a.breed + '</div></div>').join('');
