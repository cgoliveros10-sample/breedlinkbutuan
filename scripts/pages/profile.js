...
function updateProfileUI() {
    ...
    if (tagsContainer) {
        ...
        ).join('') + (isEditMode && (profileData.tags || []).length < MAX_TAGS ? '<button class="add-tag-btn" onclick="addNewTag()">➕ Add Tag</button>' : '');
    }
...
function addNewTag() {
    if (!isEditMode) { showToast('Please click Customize Profile first to edit', 'error'); return; }
    if ((profileData.tags || []).length >= MAX_TAGS) {
        showToast(`You can only add up to ${MAX_TAGS} tags.`, 'error');
        return;
    }
    openModal('tagModal');
}
