...
function openBPPostDetail(userId, postId, imagesJson) {
    ...
    (async () => {
        const [{ data: postData }, { data: likeRow }] = await Promise.all([
            window.supabase.from('posts').select('*').eq('id', postId).single(),
    ...
    window._bppdToggleLike = async (uid, pid) => {
        ...
        try {
            ...
            if (_bppdIsLiked) {
                ...
                await window.supabase.from('posts').update({ likes: Math.max(0, currentLikes - 1) }).eq('id', pid);
                _bppdIsLiked = false;
                ...
                if (error && error.code !== '23505') throw error;
                await window.supabase.from('posts').update({ likes: currentLikes + 1 }).eq('id', pid);
                _bppdIsLiked = true;
